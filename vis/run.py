class Run():
    def __init__(self, conn):
        ...
        #get all other stuff
        sql = "SELECT pop_size, ga_steps, reg_steps, mut_prob, mut_prob_limit, mut_step, cross_frac, cross_frac_limit, cross_step, num_genes, gene_bits, min_protein_conc, max_protein_conc, alpha, beta, decay_rate, initial_proteins, max_proteins, max_mut_float, max_mut_bits, fitness_log_interval, binding_method, graph_results, log_grns, log_reg_steps, growth_start, growth_end, growth_sample_interval, growth_seq, growth_threshold, term_cutoff FROM run;"
        
        rs = conn.select(sql, [], [int, int, int, float, float, float, float, float, float, int, int, float, float, float, float, float, int, int, float, int, int, str, int, int, int, int, int, int, str, float, float])
